                        ...
                        <tbody id="productsTableBody">
                            @forelse($products as $product)
                                <tr>
                                    <td>
                                        @if($product->image)
                                            <img src="{{ asset($product->image) }}" alt="{{ $product->name }}"
                                                style="width: 50px; height: 50px; border-radius: 8px; object-fit: cover; border: 1px solid var(--border-subtle);">
                                        @else
                                            <div
                                                style="width: 50px; height: 50px; border-radius: 8px; background: #100F0E; border: 1px solid var(--border-subtle); display: flex; align-items: center; justify-content: center; color: var(--gold); font-family: var(--font-display); font-weight: bold;">
                                    ...
                                    <td><span
                                            class="badge">{{ $product->category ? $product->category->category_name : 'Uncategorized' }}</span>
                                    </td>
                                    <td>
                                        {{ number_format($product->price, 2) }}
                                        @if($product->discount_price)
                                            <br><small style="color:var(--gold);">Discount:
                                                {{ number_format($product->discount_price, 2) }}</small>
                                        @endif
                                    </td>
                                    <td>{{ $product->stock }}</td>
                                    <td style="white-space: nowrap;">
                                        <button class="btn-outline btn-sm edit-product" data-id="{{ $product->id }}"
                                        ...
                                        <form action="{{ route('products.destroy', $product->id) }}" method="POST"
                                            ...
                                            @csrf
                                            @method('DELETE')
                                            <button type="submit" class="btn-danger-sm delete-product"
                                                onclick="return confirm('Are you sure you want to remove this product?');"><i
        ...
        <div class="form-modal">
            ...
            <form id="productForm" action="{{ route('products.store') }}" method="POST" enctype="multipart/form-data">
                ...
                <div class="form-group">
                    ...
                    <textarea name="description" id="prodDesc" rows="2"
                        placeholder="Luxury fabric, elegant design..."></textarea>
                </div>
                <div class="form-group">
